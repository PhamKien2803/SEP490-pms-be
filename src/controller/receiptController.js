...
exports.confirmReceiptController = async (req, res) => {
    try {
        ...
        if (!receipt) {
            return res.status(HTTP_STATUS.NOT_FOUND).json({ message: "Không tìm thấy biên lai" });
        }
        receipt.state = "Đã xác nhận";
        await receipt.save();
        return res.status(HTTP_STATUS.OK).json({ message: "Xác nhận biên lai thành công" });
    } catch (error) {
